...
!(function($) {
    var pluginName = "ajaxImageUpload",
        defaults   = {
            ...
            postUrl: '', // POST上传地址
            auto: true, // 设置为 true 后，有文件选择即开始上传
            uploadButton: true, // 图片达到最多允许的上传个数后，是否显示上传按钮
            width: 150, // 图片宽度
            height: 150, // 图片高度
            imageUrl: [], // 已上传的图片连接
            postData: {}, // 额外数据
    ...
    Plugin.prototype = {
        ...
        createSectionBox: function() {
            var $self = this,
                ...
                $uploadInput   = $("<input type='file' name='" + $fileInput + "_tmp[]' class='ggy-upload-input' accept='image/*' multiple='multiple'>");
        ...
        createImageSection: function($src) {
            var $self = this,
                ...
                $uploadInput  = $this.find("input[type=file]"),
                $imageSection1 = $this.find('.ggy-image-section'),
                $maxNum = $self.settings.maxNum,
                $uploadButton = $self.settings.uploadButton;
            ...
            switch ($appendMethod) {
                case "before":
                    ...
                    // 图片达到最多允许的上传个数后，是否显示上传按钮                    
                    if (!$uploadButton) {
                        var $files = $uploadInput[0].files;
                        if ($imageSection1.length != 0) {
                            var $is = $imageSection1.length + 1;
                        }

                        if ($files.length == $maxNum || $is >= $maxNum) {
                            $uploadSection.hide();
                        }
                    }
                    break;
        ...
        */
        getBase64Image: function($file) {
            ...
            // 读取指定的文件内容，并将其编码成Base64格式的数据URL
            reader.readAsDataURL($file);
            reader.onload = function (event) {
                var $base64 = event.target.result;
                // console.log($base64);
                // return $base64;
                $imageShow.removeClass("shade").attr('src', $base64);
            }
            $imageShow.removeClass("shade").attr('src', $base64);
            // $imageSection.find('input[type=hidden]').val($base64);
